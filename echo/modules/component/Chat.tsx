...
export default function Chat() {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            // Shape a compact history for the API
            ...
            const res = await fetch("/api/chat", {
                ...
                body: JSON.stringify({ history, question: userMessage.text }),
            });
            ...
            // Accept either {answer} or {reply} or {text}
            const aiText: string =
                data.answer ?? data.reply ?? data.text ?? "Sorry, I couldn't generate a reply.";
    ...
    return (
        <div className="flex h-screen bg-linear-to-br from-[#7B93DB] via-[#8DA8E3] to-[#9DB3E8]">
            ...
            {/* Sidebar */}
            <aside
                className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col justify-between border-r border-white/20 bg-linear-to-b from-white/10 to-white/5 backdrop-blur-md px-6 py-8 shadow-2xl transition-transform duration-300 ease-in-out md:static md:z-auto md:w-64 md:translate-x-0 lg:w-72 xl:w-[280px] ${
                        ...
                        {navItems.map((item, index) => (
                            <button
                                ...
                                className={`group flex items-center gap-3 rounded-lg px-4 py-3 text-left transition-all duration-200 ${
                                    item.active ? "bg-white/20 shadow-lg backdrop-blur-sm" : "hover:bg-white/10 hover:shadow-md"
                                }`}
                    ...
                    <button className="group flex items-center justify-between rounded-lg p-2 transition-all hover:bg-white/10">
                        <div className="flex items-center gap-3">
                            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/20 shadow-lg backdrop-blur-sm transition-transform group-hover:scale-105">
                                <span className="font-['Source_Sans_3'] text-lg font-semibold text-white">KI</span>
                            </div>
            ...
            <main className="flex flex-1 flex-col">
                ...
                <header className="flex justify-end gap-3 px-4 py-4 pt-16 md:px-8 md:py-8 md:pt-8 lg:px-12 lg:py-10 xl:px-16 xl:py-12">
                    ...
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-linear-to-r from-[#7B93DB] to-[#9DB3E8] shadow-lg ring-2 ring-white/30 transition-transform hover:scale-110">
                        <span className="font-['Roboto'] text-sm font-bold text-white">KI</span>
                    </div>
                </header>
                                            ...
                                            <div className={`flex max-w-[85%] gap-3 ${msg.sender === "user" ? "flex-row-reverse" : "flex-row"}`}>
                                                ...
                                                <div
                                                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
                                                        msg.sender === "user" ? "bg-linear-to-r from-[#7B93DB] to-[#9DB3E8]" : "bg-linear-to-r from-purple-500 to-pink-500"
                                                    } shadow-lg`}
                                                >
...
function AddIcon({ active }: { active: boolean }) {
    return (
        <svg width="24" height="24" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M12.7279 15.7279L12.7279 9.72792" stroke={active ? "white" : "white"} strokeOpacity={active ? "1" : "0.9"} strokeWidth="2.2" strokeLinecap="round" />
            <path d="M15.7279 12.7279L9.72791 12.7279" stroke={active ? "white" : "white"} strokeOpacity={active ? "1" : "0.9"} strokeWidth="2.2" strokeLinecap="round" />
            <path d="M17.5844 5.75015C14.2698 3.43752 9.67507 3.75998 6.71753 6.71752C3.39807 10.037 3.39807 15.4189 6.71753 18.7383C10.037 22.0578 15.4189 22.0578 18.7383 18.7383C21.6959 15.7808 22.0183 11.186 19.7057 7.87147" stroke={active ? "white" : "white"} strokeOpacity={active ? "1" : "0.9"} strokeWidth="2" strokeLinecap="round" />
        </svg>
    );
}
    ...
    return (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
                ...
                stroke="#3D2D4C" strokeWidth="2" strokeLinecap="round"
            />
        </svg>
    );
}
